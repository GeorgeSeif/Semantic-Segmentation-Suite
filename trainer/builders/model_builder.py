...
sys.path.append("trainer/models")
...
def download_checkpoints(model_name):
    subprocess.check_output(["python", "trainer/utils/get_pretrained_checkpoints.py", "--model=" + model_name])



def build_model(model_name, net_input, num_classes, image_width, image_height, frontend="ResNet101", is_training=True):
	...
	if "ResNet50" == frontend and not os.path.isfile("trainer/models/resnet_v2_50.ckpt"):
	    download_checkpoints("ResNet50")
	
	if "ResNet101" == frontend and not os.path.isfile("trainer/models/resnet_v2_101.ckpt"):
	    download_checkpoints("ResNet101")
	
	if "ResNet152" == frontend and not os.path.isfile("trainer/models/resnet_v2_152.ckpt"):
	    download_checkpoints("ResNet152")
	
	if "MobileNetV2" == frontend and not os.path.isfile("trainer/models/mobilenet_v2.ckpt.data-00000-of-00001"):
	    download_checkpoints("MobileNetV2")
	
	if "InceptionV4" == frontend and not os.path.isfile("trainer/models/inception_v4.ckpt"):
	...
	if model_name == "FC-DenseNet56" or model_name == "FC-DenseNet67" or model_name == "FC-DenseNet103":
	    ...
	    network, init_fn = build_pspnet(net_input, label_size=[image_height, image_width], preset_model = model_name, frontend=frontend, num_classes=num_classes, is_training=is_training)
