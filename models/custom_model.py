...
def build_custom(inputs, num_classes, frontend="ResNet101", weight_decay=1e-5, is_training=True, pretrained_dir="models"):
	...
	up_1 = conv_transpose_block(end_points["pool1"], strides=2, n_filters=64)
	up_2 = conv_transpose_block(end_points["pool2"], strides=4, n_filters=64)
	up_3 = conv_transpose_block(end_points["pool3"], strides=8, n_filters=64)
	up_4 = conv_transpose_block(end_points["pool4"], strides=16, n_filters=64)
	up_5 = conv_transpose_block(end_points["pool5"], strides=32, n_filters=64)

	features = tf.concat([up_1, up_2, up_3, up_4, up_5], axis=-1)

	features = conv_block(inputs=features, n_filters=64, filter_size=[1, 1])
